import { check, define, err, guard, isOk, pass, scenario, step } from 'seizu';
...
import {
  ...
  parseScenarios,
} from '../doc/parser/scenario-parser';
...
export const docParse = define<DocPipelineState, ParseInput, PipelineError>(
  'doc.parse',
  {
    pre: [
      ...
      guard('scenario flow must be deterministic', (_, input) => {
        for (const entry of input.sourceFiles) {
          if (entry.kind !== 'scenario') continue;
          const violation = collectScenarioFlowViolations(entry.sourceFile)[0];
          if (violation) {
            return err({ tag: 'DynamicScenarioFlow' as const, ...violation });
          }
        }
        return pass;
      }),
