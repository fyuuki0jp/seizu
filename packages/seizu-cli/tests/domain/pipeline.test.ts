...
import { expectErr, expectOk } from 'seizu/testing';
...
const dynamicScenarioSource = `
import { define, scenario, step } from 'seizu';
...
const dynamicFlow = scenario('cart.dynamicFlow', {
  flow: () => {
    const steps = [];
    steps.push(step(createCart, { userId: 'alice' }));
    return steps;
  },
});
`;

const nonDirectStepScenarioSource = `
import { define, scenario, step } from 'seizu';
...
const helperFlow = scenario('cart.helperFlow', {
  flow: () => [
    buildStep(createCart, { userId: 'alice' }),
  ],
});
`;
...
function makeDynamicScenarioEntries(): SourceFileEntry[] {
  return [
    {
      path: 'dynamic.ts',
      kind: 'scenario',
      sourceFile: createSourceFile('dynamic.ts', dynamicScenarioSource),
    },
  ];
}

function makeNonDirectStepScenarioEntries(): SourceFileEntry[] {
  return [
    {
      path: 'nondirect.ts',
      kind: 'scenario',
      sourceFile: createSourceFile('nondirect.ts', nonDirectStepScenarioSource),
    },
  ];
}
...
describe('doc.parse', () => {
  ...
  test('rejects dynamic scenario flow patterns', () => {
    const state = makeInitialState();
    const error = expectErr(
      docParse(state, { sourceFiles: makeDynamicScenarioEntries() })
    );
    expect(error.tag).toBe('DynamicScenarioFlow');
    if (error.tag === 'DynamicScenarioFlow') {
      expect(error.scenarioName).toBe('cart.dynamicFlow');
    }
  });

  test('rejects non-direct step elements in scenario flow arrays', () => {
    const state = makeInitialState();
    const error = expectErr(
      docParse(state, { sourceFiles: makeNonDirectStepScenarioEntries() })
    );
    expect(error.tag).toBe('DynamicScenarioFlow');
    if (error.tag === 'DynamicScenarioFlow') {
      expect(error.scenarioName).toBe('cart.helperFlow');
    }
  });
  ...
  test('exposes contract metadata', () => {
    ...
    expect(docParse.pre.length).toBe(2);
  });
});
...
describe('doc.generate scenario', () => {
  ...
  test('fails when dynamic scenario flow is included', () => {
    const state = makeInitialState();
    const failure = expectErr(
      docGenerate(state, {
        sourceFiles: makeDynamicScenarioEntries(),
        coverageEnabled: false,
      })
    );

    expect(failure.stepIndex).toBe(0);
    expect(failure.contractName).toBe('doc.parse');
    expect(failure.error.tag).toBe('DynamicScenarioFlow');
  });
  ...
  test('parity: matches existing generate() output', () => {
    ...
    // Now run the same through seizu pipeline
